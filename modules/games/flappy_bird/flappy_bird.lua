...
function game:init() -- Init the screen, can be left as is
    local rootWidget = self.arcade.object.uiComponent:GetGameController():GetRootCompoundWidget()
...
function game:showDefault() -- Show the default home screen
    ...
    ink.text("Fappy", 40, 0, 50, HDRColor.new({ Red = 1, Green = 1, Blue = 0, Alpha = 1.0 })):Reparent(area, -1)
    ink.text("Berd", 100, 50, 40, HDRColor.new({ Red = 1, Green = 0, Blue = 0, Alpha = 1.0 })):Reparent(area, -1)
    ...
    ink.text("Play", 0, 0, 25):Reparent(buttons, -1)
...
function game:handleMenuInput(key)
	...
	if key == "interact" then
		self.selectorInk:SetTintColor(HDRColor.new({ Red = 0, Green = 0.8, Blue = 0.8, Alpha = 1 }))
		Cron.After(0.1, function ()
			self.selectorInk:SetTintColor(HDRColor.new({ Red = 0, Green = 0.5, Blue = 0.5, Alpha = 1 }))
		end)
...
function game:initGame()
	...
	self.gameText = ink.text("GAME ", 35, 90, 55, HDRColor.new({ Red = 1, Green = 0, Blue = 0, Alpha = 1 }))
	self.gameText:SetVisible(false)
	self.gameText:Reparent(self.gameScreen, -1)

	self.overText = ink.text("OVER", 175, 90, 55, HDRColor.new({ Red = 1, Green = 0, Blue = 0, Alpha = 1 }))
	self.overText:SetVisible(false)
	self.overText:Reparent(self.gameScreen, -1)

	self.scoreText = ink.text("", 100, 140, 25, HDRColor.new({ Red = 0, Green = 1.25, Blue = 0, Alpha = 1 }))
	self.scoreText:SetVisible(false)
	self.scoreText:Reparent(self.gameScreen, -1)

	self.continueText = ink.text("Press JUMP to continue", 100, 180, 20, HDRColor.new({ Red = 0, Green = 0, Blue = 1, Alpha = 1 }))
	self.continueText:SetVisible(false)
